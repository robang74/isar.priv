...
deb_lists_dir_import() {
    ...
    export dn="${1}/var/lib/apt/lists/"
    flock -s "${pc}".lock -c 'sudo -s << EOSUDO
        set -e
        mkdir -p "${dn}" && test -d "${dn}"
        find "${pc}" -type f -not -name lock -maxdepth 1 -not -name \
            _isar-apt\* -exec ln -Pf -t "${dn}" {} + 2>/dev/null || :
...
deb_lists_dir_export() {
    export pc="${DEBDIR}/lists/${2}"
    export dn="${1}/var/lib/apt/lists/"
    flock "${pc}".lock -c 'sudo -Es << EOSUDO
        set -e
        mkdir -p "${pc}" && test -d "${pc}"
        find "${dn}" -type f -not -name lock -maxdepth 1 -not -name \
            _isar-apt\* -exec ln -Pf -t "${pc}" {} + 2>/dev/null || :
...
deb_dl_dir_import() {
    export pc="${DEBDIR}/${2}"
    export dn="${1}/var/cache/apt/archives/"
    flock -s "${pc}".lock -c 'sudo -Es << EOSUDO
        set -e
        mkdir -p "${dn}" && test -d "${dn}"
        find "${pc}" -maxdepth 1 -type f -iname "*\.deb" \
...
deb_dl_dir_export() {
    ...
    flock "${pc}".lock -c 'sudo -Es << EOSUDO
        set -e
        mkdir -p "${pc}" && test -d "${pc}"
        find "${dn}" -maxdepth 1 -type f -iname '*\.deb' |\
